...
public class BaseProxy extends BaseClass{
    ...
    static public void startProxy(){
        ...
        ConfigUserAgens.createUserAgent(USERAGENT.ANDROIDFR,serverBase);
    }
    @AfterClass
    static public void stopProxy() throws IOException {
        ...
        har.writeTo(new File("site"+timeStamp+".har"));//записываем хар файл
// Some code here ... (close browser, stop server)
        serverBase.stop();
        for (HarEntry entry : har.getLog().getEntries()) {
            //  System.out.println(entry);
//           System.out.print(" "+entry.getTimings().getWait());
//           System.out.println(" "+entry.getTimings().getReceive());//время прогрузок в консоль
